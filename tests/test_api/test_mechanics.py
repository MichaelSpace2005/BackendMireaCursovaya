...
async def test_create_and_get_tree(client):
    r1 = await client.post(
        "/api/v1/mechanics", json={"name": "Jump", "description": "player jump", "year": 1978}
    )
    ...
    r2 = await client.post(
        "/api/v1/mechanics",
        json={"name": "Double Jump", "description": "player double jump", "year": 1990},
    )
    assert r2.status_code == 201
    m2 = r2.json()

    rlink = await client.post(
        "/api/v1/links",
